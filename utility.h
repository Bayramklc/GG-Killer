/* this is sort of just a file (2 files if you include the ntdll library) 
that are passed around between programs for common functions that are used. */
...
/* in case of logging/configuring. path_... functions need this */
#define PROGRAM_ALIAS "GameGuard"

/* base address of some common modules, in order of initialization, some of them might be 0 as they are not yet loaded */
#define NTDLL GetModHandleA("ntdll.dll")
#define KERNEL32 GetModHandleA("kernel32.dll")
#define USER32 GetModHandleA("user32.dll")
#define ADVAPI GetModHandleA("advapi32.dll");

#pragma region Native Defines
...
uint32_t GetDWORD(byte *b)
{
	return b[0] | b[1] << 8 | b[2] << 16 | b[3] << 24;
}

uint32_t GetWORD(byte *b)
{
	return b[0] | b[1] << 8;
}
...
mem_result mem_test(uint32_t dwBaseAddress, uint32_t dwBufferLength)
{
	...
	__try
	{
		...
		for (uint32_t i = 0; i < dwBufferLength; i++)
		{
...
mem_result mem_test(uint32_t dwBaseAddress, uint32_t* pdwOffsets, uint32_t dwOffsetCount, uint32_t dwBufferLength)
{
	mem_result _result = { 0 };
	_result.address = dwBaseAddress;
	_result.length = dwBufferLength;
	...
	__try
	{
		...
		_result.address = *(uint32_t*)dwBaseAddress;
		for (unsigned int i = 0; i < dwOffsetCount - 1; i++)
		{
			...
			_result.address = *(uint32_t*)(_result.address + pdwOffsets[i]);
		}

		_result.address = (uint32_t)_result.address + pdwOffsets[dwOffsetCount - 1];
...
char* mem_get_str(uint32_t memAddress, unsigned int* ptrOffsets, unsigned int offsetCount, uint32_t bufferSize)
{
	char* retn = (char*)malloc(bufferSize);
	if (!memAddress)
		return 0;

	/* unsafe, can cause crash */
	uint32_t address = *(uint32_t*)memAddress;
	for (UINT i = 0; i < offsetCount - 1; i++)
	{
		...
		/* unsafe, can cause crash */
		address = *(uint32_t*)(address + ptrOffsets[i]);
	}
	...
	strncpy(retn, (char*)address + ptrOffsets[offsetCount - 1], bufferSize);
	return retn;
}

char* mem_get_cstr(uint32_t memAddress, uint32_t bufferSize)
{
	char* retn = (char*)malloc(bufferSize);
	strncpy(retn, (char*) *(uint32_t*)memAddress, bufferSize);
	return retn;
}

const char* mem_read_cstr(uint32_t memAddress, unsigned int* ptrOffsets, unsigned int offsetCount)
{
	if (!memAddress)
		return 0;
...
const char* mem_read_cstr(uint32_t memAddress)
{
	return (const char*) *(uint32_t*)memAddress;
}

int mem_read_int(uint32_t memAddress, unsigned int* ptrOffsets, unsigned int offsetCount)
{
	if (!memAddress)
		return 0;
...
int mem_read_int(uint32_t baseAddress)
{
	return (int)*(uint32_t*)baseAddress;
}

uint32_t mem_get_ptr(uint32_t baseAddress, uint32_t* offsets, unsigned int offsetCount)
{
	if (!baseAddress)
		return baseAddress;

	/* unsafe, can cause crash */
	uint32_t address = *(uint32_t*)baseAddress;
	for (unsigned int i = 0; i < offsetCount - 1; i++)
	{
		...
		/* unsafe, can cause crash */
		address = *(uint32_t*)(address + offsets[i]);
	}

	if (!address)
		return 0;

	return (uint32_t)address + offsets[offsetCount - 1];
}

void mem_write(PVOID pAddress, uint32_t dwLen, BYTE* ptrWrite)
{
	DWORD dwProtection;
...
void mem_nop(PVOID pAddress, uint32_t dwLen)
{
	DWORD dwProtection;
...
uint32_t mem_ptn_find(const char* pattern, const char* mask, MODULEINFO module)
{
	/* specifies where the function will start searching from */
	uint32_t base = (uint32_t)module.lpBaseOfDll;

	/* specifies where the function will end searching */
	uint32_t size = (uint32_t)module.SizeOfImage;

	/* loop through the specified module */
	for (uint32_t retAddress = base; retAddress < base + size - strlen(mask); retAddress++)
	{
		if (*(BYTE*)retAddress == (pattern[0] & 0xff) || mask[0] == '?')
		{
			uint32_t startSearch = retAddress;
...
int* strtodwarray(char* str, const char delim)
{
	int* retn;
	if (str)
	{
		/* split the string */
		char** split = str_split(str, delim);
		if (!split)
		{
			return 0;
		}
		...
		uint32_t count = 0;
		for (uint32_t i = 0; *(split + i); i++)
		{
			count++;
		}

		/* prepare the return */
		retn = (int*)malloc(sizeof(int) * count);
...
char** dwtostrarray(int arr[], uint32_t count)
{
	/* max size of a 32bit int string */
	int single = sizeof(char) * 9 + 1;

	/* the length of all of the strings combined */
	int length = single * count;

	/* prepare the array */
	char** retn = (char**)malloc(length);

	/* loop through with the given count */
	for (uint32_t i = 0; i < count; i++)
	{
		/* allocate a single string */
		char* current = (char*)malloc(single);

		/* paste the contents of the current element to the string */
		sprintf(current, "%d", arr[i]);
		current[9] = 0;

		/* move the string into the array */
		memcpy(&retn[i], current, single);

		/* should be removed I think */ retn[i] = current;
		free(current);
	}
	return retn;
}

char* substr(const char* str, int index, int length, size_t lpSize = 256) 
{
	/* prepare the buffer */
	char* buffer = (char*)malloc(lpSize);

	/* create the substring */
	strncpy(buffer, str + index, length);
	buffer[length] = '\0';
	return buffer;
}

int strindex(const char* str, char delim)
{
	const char* pch = strrchr(str, delim);
	return pch ? pch - str + 1 : -1;
}

bool strnull(const char* str)
{
...
void strlow(char* str)
{
	for (int i = 0; str[i]; i++) 
	{
		/* converts the string's character to their lower case */
		str[i] = tolower(str[i]);
	}
}
...
void strup(char* str)
{
	/* converts the string's characters to their upper case */
	for (int i = 0; str[i]; i++) 
	{
		str[i] = toupper(str[i]);
	}
...
#pragma region Custom Windows API functions
...
FARPROC GetProcedureAddress(HANDLE hModule, char* pszProcName)
{
	...
	if (!pNames)
	{
		/* the address of the names is not available */
		return 0;
	}
...
HMODULE GetModHandleW(LPCWSTR lpModuleName)
{
	PPEB pPeb = NtCurrentPeb();
	...
	PLIST_ENTRY pListEntryFirst = &(pPeb->LoaderData->InMemoryOrderModuleList);

	/* loop through entry list till we find a match for the module's name */
	for (PLIST_ENTRY pListEntry = pListEntryFirst->Flink; pListEntry != pListEntryFirst; pListEntry = pListEntry->Flink)
	{
		PLDR_DATA_TABLE_ENTRY pLdrDataTableEntry = (PLDR_DATA_TABLE_ENTRY)(pListEntry);

		/* important note is that this is strict to the entire name, case sensitive. */
		if (!lstrcmpiW(lpModuleName, pLdrDataTableEntry->FullModuleName.Buffer))
		{
			/* this is it's base address */
...
HMODULE GetModHandleA(LPCSTR lpModuleName)
{
	...
	/* allocate for the unicode name and the null terminating character */
	WCHAR* moduleNameW = (WCHAR *)malloc((strlen(lpModuleName) + 1) * sizeof(WCHAR));

	/* get the unicode string */
	if (MultiByteToWideChar(CP_ACP, 0, lpModuleName, -1, moduleNameW, (strlen(lpModuleName) + 1)) == 0)
	{
		return 0;
	}

	/* call the unicode function */
	HMODULE moduleBase = GetModHandleW(moduleNameW);
...
DWORD NtSyscallIndex(char* szFunctionName)
{
	BYTE buffer[5];
	...
	return GetDWORD(buffer + 1);
}

#pragma endregion

#pragma region Envinroment

char* path_main_module()
{
...
char* path_local()
{
	char* module = path_main_module();
	int index = strindex(module, '\\');
	char* retn = substr(module, 0, index);
	free(module);
...
size_t file_size(const char* name)
{
	...
	/* go to the end of the file and ask for the position */
...
void LOG_NORMAL(const char* input, ...)
{
#ifdef PROGRAM_ALIAS
	char* path = path_log_file();
...
void LOG_ERROR(const char* input, ...) 
{
#ifdef PROGRAM_ALIAS
	char* path = path_log_file();
	FILE* iobuf = fopen(path, "ab+");
